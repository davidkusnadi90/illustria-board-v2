...
export default async function LaporanTHR2026Page() {
  /**
  ...
  const paidPayments =
    payments?.filter((p) => p.paid).map((p) => {
      const addr = Array.isArray(p.cluster_addresses)
        ? p.cluster_addresses[0]
        : p.cluster_addresses
  ...
  /**
   * 4. Determine which addresses already have ANY payment record
   */
  ...
  /**
   * 5. Build UNPAID list
   *    = addresses with NO payment record at all
   */
    ...
    <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
      ...
      <header className="space-y-3">
        ...
        <p className="text-sm text-gray-600">
          Transparansi pembayaran THR warga tahun 2026.
        </p>
      </header>
      ...
      <section className="rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 p-5 text-white shadow">
        <p className="text-sm text-white/90">
          Total Dana THR Diterima
        </p>
        <p className="mt-2 text-2xl font-bold">
          Rp {totalReceived.toLocaleString('id-ID')}
        </p>
        <p className="mt-1 text-xs text-white/80">
          Akumulasi pembayaran THR yang sudah diterima
        </p>
      </section>
